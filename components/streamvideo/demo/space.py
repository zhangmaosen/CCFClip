...
```bash
pip install gradio_streamvideo
```
...
""", elem_classes=["md-custom"], header_links=True)
    ...
    gr.ParamViewer(value=_docs["StreamVideo"]["members"]["__init__"], linkify=[])


    gr.Markdown("### Events")
    gr.ParamViewer(value=_docs["StreamVideo"]["events"], linkify=['Event'])
...
def predict(
    ...
    return value
```
""", elem_classes=["md-custom", "StreamVideo-user-fn"], header_links=True)
