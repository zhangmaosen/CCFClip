...
with gr.Blocks(fill_width=True, css=".srt_file {height:80px}") as demo:
    user_name = gr.Textbox(label="input")

    @gr.render(inputs=user_name)
    def next_step(text):
       
        wk_data = load_workspace("zms", db_name="ccfclip")
        ...
        with gr.Row():
            ...
            model_select = gr.Dropdown(["qwen2.5:72b-instruct","qwen2:72b-instruct","qwen2:7b-instruct","phi3:14b-instruct","llama3.1:70b-instruct-q2_K", "gemma2:27b-instruct-q4_0", "deepseek-v2:16b"], label="Model", value=llm_config.get("model_name",""))
            temperature = gr.Slider(minimum=0.0, maximum=1.0, step=0.1, label="Temperature", value=llm_config.get("temperature", 0.1))
            num_predict = gr.Slider(minimum=0, maximum=10000, step=1, label="Number of Predictions", value=llm_config.get("prediction_num", 1024))
...
#     return e.value
    ...
    s_video = StreamVideo(visible=False)# def save_workspace(model_select, temperature, context_length,  keep_alives, num_predict,
