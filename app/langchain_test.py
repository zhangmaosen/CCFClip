...
collection = chroma_client.get_or_create_collection("srts2")
#collection.add(ids=["1", "2", "3"], documents=["a", "b", "c"])

vector_store_from_client = Chroma(
    client=chroma_client,
    collection_name="srts2",
    embedding_function=embeddings,
)

from langchain_experimental.text_splitter import SemanticChunker
text_splitter = SemanticChunker(embeddings,sentence_split_regex="\n")
...
def index(content):
    ...
    vector_store_from_client.add_documents(chunk_docs)
    return chunk_docs

def search(query):
    results = vector_store_from_client.similarity_search_with_score(query, k=3)
    return results
...
def create(database):
    global collection , vector_store_from_client
    collection = chroma_client.get_or_create_collection(database,
...
with gr.Blocks() as demo:
    ...
    create_btn.click(create, database, None)
    srt_content = gr.Textbox(label="SRT Content",value=srt_text)
    gr.Interface(index,srt_content, "text")
    #gr.Button("Generate").click(index, srt_content )
    gr.Interface(search, "text", "text")

    gr.Interface(match, "text", "text")
demo.launch()
